...
routes.post('/Signup', async (req, res) => {
    try {
        ...
        const newUser = new User({ name: name, email: email, hashPassword: hashPassword, role: role, date: Date.now() })
        console.log(newUser);
...
routes.get('/logout', isAuth, async (req, res) => {
    try {
        ...
        res.status(200).send({ message: 'User signed-out!', redirectUrl: "/Login" });
    } catch (error) {
        res.status(500).json({ message: 'Server error' });
    }
})
